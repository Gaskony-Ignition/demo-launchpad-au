...
them — earlier versions leave inputs and dropdowns with no visible border. Ignition's
own six themes are unaffected and need nothing.

---

## It runs on Ignition Edge too

Both projects ship a second zip built for **Ignition Edge**, which has no
database connectivity of any kind. The screens are the same ones: OEE's records
and KPI's dashboards are kept in the gateway's own data directory instead of in
SQL, and KPI's tag history is seeded through `system.historian` rather than by
writing the historian's tables.

The install differs in two ways there — Edge has no project import UI, and one
gateway setting has to change before any Perspective session will open at all.
**[docs/EDGE.md](docs/EDGE.md)** covers both, and what the two builds do and do
not share.
...
The Apache-2.0 licence in [LICENSE](LICENSE) covers the changed and added work in this
